...
func (s *testSuite) TestGlobalAndSessionBindingBothExist(c *C) {
	...
	tk.MustQuery("explain SELECT /*+ HASH_JOIN_HASHER(t2) */ * from t1,t2 where t1.id = t2.id").Check(testkit.Rows(
		"HashLeftJoin_8 12487.50 root inner join, inner:TableReader_15, hasher:inner, equal:[eq(test.t1.id, test.t2.id)]",

...
func (s *testRangerSuite) TestCompIndexInExprCorrCol(c *C) {
	...
	testKit.MustExec("analyze table t")
	testKit.MustQuery("explain select /*+ HASH_JOIN_HASHER(t) */ t.e in (select count(*) from t s use index(idx), t t1 where s.b = 1 and s.c in (1, 2) and s.d = t.a and s.a = t1.a) from t").Check(testkit.Rows(
		"Projection_11 2.00 root 9_aux_0",
		"└─Apply_13 2.00 root CARTESIAN left outer semi join, inner:StreamAgg_20, hasher:inner, other cond:eq(test.t.e, 7_col_0)",
